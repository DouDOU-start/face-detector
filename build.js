...
async function run() {
  ...
  if (fs.existsSync(modelsDir)) {
    ...
    if (fs.existsSync(modelJsonPath)) {
      try {
        ...
        // Also prepare artifacts for generating an embeddable CommonJS module for bundlers
        embeddedArtifacts = {
          format: 'graph-model',
          generatedBy: 'custom-embed',
          convertedBy: 'tfjs-converter',
  ...
  // Output 3: generate a CommonJS module exporting an IOHandler for npm bundlers (offline model)
  if (embeddedArtifacts) {
    const embeddedModulePath = path.join(modelsDir, 'embedded.js');
    const cjs = `// Auto-generated by build.js. Do not edit manually.\n` +
`const artifacts = ${JSON.stringify(embeddedArtifacts)};\n` +
`function b64ToArrayBuffer(b64){\n  if (typeof atob === 'function') {\n    const bin = atob(b64);\n    const len = bin.length;\n    const bytes = new Uint8Array(len);\n    for (let i=0;i<len;i++) bytes[i] = bin.charCodeAt(i);\n    return bytes.buffer;\n  } else if (typeof Buffer !== 'undefined') {\n    return Buffer.from(b64, 'base64').buffer;\n  } else {\n    throw new Error('No base64 decoder available');\n  }\n}\n` +
`module.exports = {\n  load: async function(){\n    return {\n      format: artifacts.format,\n      generatedBy: artifacts.generatedBy,\n      convertedBy: artifacts.convertedBy,\n      modelTopology: artifacts.modelTopology,\n      weightSpecs: artifacts.weightSpecs,\n      weightData: b64ToArrayBuffer(artifacts.weightDataBase64)\n    };\n  }\n};\n`;
    fs.writeFileSync(embeddedModulePath, cjs, 'utf8');
    console.log('Generated embedded IOHandler module ->', path.relative(process.cwd(), embeddedModulePath));
  }
}
